...
import type { Objeto, SaudeObjeto } from "@/types";
...
const SAUDE_CONFIG: Record<SaudeObjeto, { dot: string; label: string; bar: string }> = {
...
interface ObjetoDetail extends Objeto {
...
const DetalheObjeto = () => {
  ...
  const { data: objeto, isLoading: objetoLoading, error: objetoError } = useQuery<ObjetoDetail>({
    queryKey: ["objeto", id],
    queryFn: async () => {
  ...
  const { data: contrato } = useQuery<ContratoDetail>({
    queryKey: ["contrato", objeto?.contrato_id],
    queryFn: async () => {
      const { data } = await api.get(`/contratos/${objeto!.contrato_id}`);
      return data;
    },
    enabled: !!objeto?.contrato_id,
  });

  if (objetoLoading) return <Skeleton />;
  if (objetoError) return (
    <div className="flex flex-col items-center justify-center py-32 text-center">
      <AlertTriangle className="h-12 w-12 text-amber-400 mb-4" />
      <p className="text-lg font-semibold text-slate-700">Objeto não encontrada.</p>
      <Link to="/objetos" className="mt-4 text-sm text-brand-700 hover:underline">← Voltar para objetos</Link>
    </div>
  );
  if (!objeto) return null;

  const saude = SAUDE_CONFIG[(objeto.saude as SaudeObjeto) || "VERDE"];
  const statusCfg = STATUS_CONFIG[objeto.status || ""] || STATUS_CONFIG.PLANEJADA;
  const StatusIcon = statusCfg.icon;
  const situacaoCfg = objeto.situacao ? (SITUACAO_LABEL[objeto.situacao] || null) : null;
  const pct = Number(objeto.percentual_executado || 0);
  const prazoLabel = objeto.vigencia_fim ? (() => {
  ...
  return (
    <div className="space-y-6">
      <Link to="/objetos" className="inline-flex items-center gap-1.5 text-sm text-slate-500 hover:text-brand-700 transition-colors"><ArrowLeft className="h-4 w-4" />Objetos</Link>
      ...
      <div className="flex flex-col gap-3">
        <div className="flex flex-wrap items-center gap-2">
          ...
          {objeto.situacao && <span className={`text-xs font-medium px-2.5 py-1 rounded-full border ${situacaoCfg?.cls || "bg-slate-100 text-slate-600 border-slate-200"}`}>{situacaoCfg?.label || objeto.situacao}{objeto.ano_referencia ? ` / ${objeto.ano_referencia}` : ""}</span>}
        ...
        <h1 className="text-xl font-bold text-slate-900 leading-snug">{objeto.titulo}</h1>
        <div className="flex flex-wrap items-center gap-4 text-sm text-slate-500">
          ...
          {objeto.orgao && <span className="flex items-center gap-1.5"><Building2 className="h-4 w-4 text-slate-400" />{objeto.orgao}</span>}
      ...
      <><div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <KPI label="Valor Inicial" value={fmtCurrency(contrato?.valor_global ?? objeto.valor_contrato)} color="slate" />
        <KPI label="Valor Final" value={fmtCurrency(contrato?.valor_final ?? objeto.valor_contrato)} sub={contrato?.valor_aditivo ? `+ ${fmtCurrency(contrato.valor_aditivo)} aditivo` : null} color={contrato && contrato.valor_final && contrato.valor_global && contrato.valor_final > contrato.valor_global ? "amber" : "slate"} />
        <KPI label="Valor Medido" value={fmtCurrency(objeto.valor_medido, "Não informado")} color="brand" />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
          ...
          <Section title="Prazos" icon={Calendar}>
            ...
            <PrazoRow label="Vigência" inicio={objeto.vigencia_inicio} dias={objeto.vigencia_dias} fim={objeto.vigencia_fim} />
            <PrazoRow label="Execução" inicio={objeto.execucao_inicio} dias={objeto.execucao_dias} fim={objeto.execucao_fim} />
          </Section>
          ...
          {contrato && (contrato.valor_aditivo || contrato.valor_reajustado || contrato.recurso_federal || contrato.recurso_estadual) && (
            <Section title="Financeiro detalhado" icon={TrendingUp}>
              ...
              {objeto.valor_medido          && <Row label="Valor medido"      value={fmtCurrency(objeto.valor_medido)} />}
              {objeto.saldo_a_medir         && <Row label="Saldo a medir"     value={fmtCurrency(objeto.saldo_a_medir)} />}
          ...
          {objeto.historico && <Section title="Histórico" icon={FileText}><p className="text-sm text-slate-700 leading-relaxed whitespace-pre-wrap">{objeto.historico}</p></Section>}
        ...
        <div className="space-y-6">
          <Section title="Situação oficial">
            <div className="space-y-3">
              ...
              {objeto.situacao && <div><p className="text-[10px] font-semibold text-slate-400 uppercase tracking-wide mb-1.5">Situação (planilha oficial)</p><span className={`inline-flex text-xs font-semibold px-2.5 py-1.5 rounded-xl border ${situacaoCfg?.cls || "bg-slate-100 text-slate-600 border-slate-200"}`}>{situacaoCfg?.label || objeto.situacao}{objeto.ano_referencia ? ` / ${objeto.ano_referencia}` : ""}</span>{objeto.situacao_origem && <p className="text-xs text-slate-400 mt-1.5 italic">&quot;{objeto.situacao_origem}&quot;</p>}</div>}
      ...
      {activeTab === "diario" && <DiarioContent objetoId={objeto.id} />}

      {activeTab === "medicoes" && <MedicoesContent objetoId={objeto.id} />}
    </div>
  );
};

export default DetalheObjeto;
